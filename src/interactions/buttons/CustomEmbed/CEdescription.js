...
module.exports = {
  ...
  async execute(interaction, client) {
    ...
    let msgEmbed = new EmbedBuilder().setColor("F4D58D").setDescription("**Entrez une nouvelle description**");
    ...
    collector.on("collect", (m) => {
      if (m.content.length >= 4096) {
        return error(interaction, "**Description trop longue.**", m);
      }

      interaction.editReply({
        embeds: [msgEmbed.setDescription(`**Ecrivez une nouvelle description.**`)],
      });
