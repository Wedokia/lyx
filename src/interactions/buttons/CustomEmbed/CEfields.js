  ...
  async execute(interaction, client) {
    ...
    buttonCollector.on("collect", (btnInt) => {
      ...
      if (btnInt.component.customId === "name") {
        ...
        interaction.editReply({
          embeds: [EmbedBuilder.from(msgEmbed).setDescription(`**En train de modifier \`${modifying}\`:**`)],
        });
      }
      ...
      if (btnInt.component.customId === "set") {
          ...
          return interaction.editReply({
            embeds: [
              EmbedBuilder.from(msgEmbed)
                .setColor("DarkRed")
                .setDescription(`${icons.warning} *Le nomnbre maximum de champs a été atteint.**`),
            ],
          });
        }
        ...
        newField;
        interaction.editReply({
          embeds: [EmbedBuilder.from(msgEmbed).setDescription(`**Ajouté ✅ **`)],
        });
      }
    });
    // util buttons --------------------------------------------------------------end
    ...
    collector.on("collect", (m) => {
      ...
      if (modifying === "name") {
        ...
        if (m.content.length > 256) return error(interaction, "Nom trop Long", m);
      ...
      if (modifying === "value") {
        ...
        if (m.content.length > 1024) return error(interaction, "Valeur trop longue", m);
