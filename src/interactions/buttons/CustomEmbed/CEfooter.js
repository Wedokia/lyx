...
module.exports = {
  ...
  async execute(interaction, client) {
    ...
    let msgEmbed = new EmbedBuilder().setColor("F4D58D").setDescription("**Entrez un Footer:**");
    ...
    collector.on("collect", (m) => {
      if (m.content.length >= 1024) {
        return error(interaction, "Footer trop long", m);
      }

      interaction.editReply({
        embeds: [msgEmbed.setDescription(`**Footer =>** \`${m.content}\``)],
      });
