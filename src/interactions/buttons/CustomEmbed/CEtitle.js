...
module.exports = {
  ...
  async execute(interaction, client) {
    ...
    let msgEmbed = new EmbedBuilder().setColor("F4D58D").setDescription("**Entrez un titre**");
    ...
    collector.on("collect", (m) => {
      if (m.content.length >= 256) {
        return error(interaction, "Titre trop long", m);
      }

      interaction.editReply({
        embeds: [msgEmbed.setDescription(`**Titre =>** \`${m.content}\``)],
      });
