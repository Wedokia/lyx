...
module.exports = {
  ...
  async execute(interaction, client) {
    ...
    let msgEmbed = new EmbedBuilder().setColor("F4D58D").setDescription("**Entrez une URL**");
    ...
    collector.on("collect", (m) => {
      ...
      if (!m.content.startsWith("http")) {
        return error(interaction, "URL invalide", m);
      }
      if (m.content.length >= 256) {
        return error(interaction, "URL trop longue", m);
      }
      if (
        !m.content.match(
      ...
      interaction.editReply({
        embeds: [msgEmbed.setDescription(`**URL =>** \`${m.content}\``)],
      });
